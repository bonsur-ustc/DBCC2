function record = DBCC2(func_num, run, alg)
...
for cur = 1 : subP_num
    ...
    while cur_gen  <= Base
        ...
        clc;
        fprintf('Basic iterations of the subproblem %d completed %4s%% \n',cur,num2str(roundn(cur_gen/Base*100,-1)));
        cur_gen = cur_gen + 1;
    end
    gen_count(cur) = gen_count(cur) + Base;
end
...
alpha = 0.1;
cycle = 1;
initial_partial = difficulty_estimated_partial(subP_num,subP,alg,cycle);
fprintf('Dynamic allocation in  %d cycle \n',cycle);
...
while remain_Gen > subP_num
    
    remain_Gen = min(floor(alpha*max_Gen), remain_Gen);
   
    for cur = 1 : subP_num
        gen = floor(partial(cur) * remain_Gen);
        
        gen_count(cur) = gen_count(cur) + gen;
    ...
    RemainedFEs = getRemainedFEs(func_num);
    if RemainedFEs <= NP*subP_num
        break;
    end
    
    cycle = cycle + 1;
    partial = difficulty_estimated_partial(subP_num,subP,alg, cycle);
    fprintf('Dynamic allocation in the %d cycle \n',cycle);
    RemainedFEs = getRemainedFEs(func_num);

    remain_Gen = floor(RemainedFEs/NP);
    if strcmp(alg, 'CSA')
        remain_Gen = floor(RemainedFEs/(NP*nc));
    end
end
